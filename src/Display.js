...
import { acceptMessageOnChannel } from "./Utils";
...
const buildMessage = async (display, message) => {
	...
	if (typeof setting.click === "function")
		element.on("click", setting.click);
...
const messageTemplate = async (display) => {
	...
	if (!template) {
		template = display.hasAttribute(ATTRIBUTE_MESSAGE_TEMPLATE) ? display.attr(ATTRIBUTE_MESSAGE_TEMPLATE) : `${SETTING.baseTemplatePath}/${SETTING.messageTemplate}`;
		template = await Template.load(new URL(template, location));
...
class Display extends Component {
	...
	static get NODENAME() { return NODENAME_DISPLAY; }
	...
	async addMessage(message) {
		...
		if (this.attr(ATTRIBUTE_MODE) == "prepend")
	...
	async removeMessage(message, triggerClose = true) {
		...
		if (message) {
			...
			if (triggerClose)
				message.trigger(EVENT_CLOSE_MESSAGE);
			else {
				message.remove();
